...
const ProductPage = async({params}: {params: {category: string}}) => {
  ...
  return (
    <div className="flex-1 flex container justify-center mt-10">
      <div className="grid grid-cols-1 md:grid-cols-2 px-12 gap-10 xl:gap-20 h-full">
        <div className="col-span-1">
          <Image src={data.imgLink2} width={600} height={200} alt={data.name}/>
        </div>
        <div className="description col-span-1">
          <h1 className="border-b-4 pb-3 text-3xl font-bold border-emerald-600">{data?.name?.toUpperCase()}</h1>
